...
export default function PostCard({ post, currentUserId, onUpdate, onDelete }: PostCardProps) {
  ...
  const handleScrap = async () => {
    const optimistic = { ...post, isScrapped: !post.isScrapped, scrapCount: post.isScrapped ? post.scrapCount - 1 : post.scrapCount + 1 }
    onUpdate(optimistic)
    try {
      if (post.isScrapped) {
        await supabase.from('post_scraps').delete()
          .eq('user_id', currentUserId).eq('post_id', post.id)
      } else {
        await supabase.from('post_scraps').insert({ user_id: currentUserId, post_id: post.id })
      }
    } catch {
      onUpdate(post)
      toast.error('잠시 후 다시 시도해주세요.')
    ...
    <article className="post-card">
      ...
      <div className="flex items-start justify-between mb-3">
        <Link href={`/community/user/${post.author?.id}`} className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-full bg-gray-200 overflow-hidden flex-shrink-0">
            {post.author?.profileImage ? (
              <Image src={post.author.profileImage} alt={post.author.nickname} width={36} height={36} className="w-full h-full object-cover" />
            ) : (
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowMoreMenu(true)} className="p-1 text-gray-400">
            <MoreHorizontal size={18} />
          </button>
        </div>
      </div>

      {/* ── Topic tags (JSONB inline) ── */}
      ...
      {/* ── AI Hashtags (JSONB inline) ── */}
      {post.aiHashtags && post.aiHashtags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2 mb-3">
          {post.aiHashtags.map((tag) => (
            <Link key={tag} href={`/community/hashtag/${encodeURIComponent(tag)}`} className="text-xs text-gray-500">
              AI: #{tag}
            </Link>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-50">
        <div className="flex items-center gap-4">
          ...
          <Link href={`/community/post/${post.id}#comments`} className="flex items-center gap-1.5 text-xs text-gray-500">
