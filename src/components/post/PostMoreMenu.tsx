...
export default function PostMoreMenu({
  post, isOwn, currentUserId, onClose, onDelete, onUpdate,
}: PostMoreMenuProps) {
  ...
  const handleDelete = async () => {
    // FR-11.8: cannot delete post under review
  ...
  // ── Block author (FR-13.3) ────────────────────────────────────────────────
  const handleBlock = async () => {
    await supabase
      .from('blocks')
      .insert({ blocker_id: currentUserId, blocked_id: post.author?.id })
    // Also unfollow if following (BR-12)
    await supabase
      .from('follows')
      .delete()
      .eq('follower_id', currentUserId)
      .eq('followee_id', post.author?.id)
  ...
  const handleReport = async () => {
    ...
    if (!error) {
      // BR-24: immediately hide from reporting user
      await supabase
        .from('hidden_posts')
    ...
    return (
      <BottomSheet onClose={onClose} title="게시글을 삭제할까요?">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-gray-200 text-sm font-medium text-gray-700">
            취소
          </button>
          <button onClick={handleDelete} className="flex-1 py-3 rounded-xl bg-red-500 text-white text-sm font-medium">
            삭제하기
          </button>
        </div>
      </BottomSheet>
  ...
  return (
    <BottomSheet onClose={onClose}>
      <div className="space-y-1">
        {isOwn ? (
          <>
            {/* FR-7.2.1 — Own post menu */}
            <button
              onClick={() => setScreen('confirmDelete')}
        ...
        <button
          onClick={onClose}
          className="w-full py-3.5 text-sm text-gray-500 mt-2"
        >
          취소
        </button>
      </div>
