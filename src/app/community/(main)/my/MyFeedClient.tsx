...
export default function MyFeedClient({ communityUser }: MyFeedClientProps) {
  ...
  const fetchScraps = useCallback(async () => {
    ...
    // BR-M02-005: hide scrapped posts from blocked users in scrap tab
    const blockedIds = await getBlockedAuthorIds(supabase, communityUser.id)
    ...
    if (data) {
      const mapped = data
        .map((row: Record<string, unknown>) => {
          ...
          if (!p) return null
          // Skip scrapped posts from blocked accounts (BR-M02-005)
          const authorId = p.author_id as string
          if (authorId && blockedSet.has(authorId)) return null
